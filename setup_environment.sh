...
#
#      PROJECT_STORAGE=/projects/<your-project>/... bash setup_environment.sh
#
#  Sets up everything needed to run llm_playground.ipynb:
#    1. the Isambard module environment (and undoes a trap it leaves behind)
#    2. caches redirected off the small home filesystem
#    3. uv (installed if missing)
#    4. the Python environment from pyproject.toml
#    5. the VS Code CLI, which tunnel.sbatch needs in order to run at all
#    6. a verification pass that fails loudly rather than silently
#
...
# --- EDIT THIS ------------------------------------------------------------
# Your project's storage. Home quotas on Isambard are small (a torch install
# plus caches is several GB; the model cache is hundreds), so all bulk goes
# here. PROJECTDIR is exported for you by the BriCS default modules.
# (resolved after `module reset` below -- that is what populates PROJECTDIR)
# --------------------------------------------------------------------------
...
# PROJECTDIR is exported by the BriCS default modules, so resolve now.
PROJECT_STORAGE="${PROJECT_STORAGE:-${PROJECTDIR:-$HOME}}"
echo "      project storage: $PROJECT_STORAGE"
case "$PROJECT_STORAGE" in
  "$HOME"|"$HOME"/*)
    echo ""
    echo "  WARNING: PROJECT_STORAGE resolved to your HOME directory."
    echo "  Home quotas on Isambard are small (often <10 GB free) and the model"
    echo "  in this sample is ~230 GiB. The download WILL fail partway and fill"
    echo "  your home directory. Re-run with your project's storage:"
    echo ""
    echo "      export PROJECT_STORAGE=/projects/<your-project>/<your-space>"
...
# ===========================================================================
# 2. Caches off the home filesystem
# ===========================================================================
echo "[3/6] redirecting caches to $PROJECT_STORAGE"
export UV_CACHE_DIR="${UV_CACHE_DIR:-$PROJECT_STORAGE/tmp/uv_cache/$USER}"
# uv's managed interpreters default to ~/.local/share/uv/python -- also bulk.
export UV_PYTHON_INSTALL_DIR="${UV_PYTHON_INSTALL_DIR:-$PROJECT_STORAGE/tmp/uv_python/$USER}"
# The model cache. If your site has a SHARED one, point at it and reuse
# weights colleagues already downloaded instead of pulling another few
# hundred GB against the project quota.
export HF_HOME="${HF_HOME:-$PROJECT_STORAGE/hf}"
mkdir -p "$UV_CACHE_DIR" "$UV_PYTHON_INSTALL_DIR" "$HF_HOME" logs

# Persist the resolved settings so other shells -- and the notebook kernel VS
# Code launches, which does NOT inherit this script's environment -- agree
# with what we chose here. `source env.sh` in any new terminal.
{
    echo "# Generated by setup_environment.sh -- regenerated on every run, so do"
    echo "# not hand-edit. Source it in any new shell:  source env.sh"
    # printf %q quotes each value safely: a path containing a space, a quote or
    # a '$' round-trips intact instead of being re-expanded when sourced.
    printf 'export PROJECT_STORAGE=%q\n'       "$PROJECT_STORAGE"
    printf 'export UV_CACHE_DIR=%q\n'          "$UV_CACHE_DIR"
    printf 'export UV_PYTHON_INSTALL_DIR=%q\n' "$UV_PYTHON_INSTALL_DIR"
    printf 'export HF_HOME=%q\n'               "$HF_HOME"
    # ~/.local/bin holds uv and is NOT on a default non-login PATH, so a shell
    # that only sources env.sh would otherwise not find `uv`.
...
Next:
  ...
  In any NEW terminal:   source env.sh

  Standalone Jupyter instead of VS Code:
     source env.sh && uv run jupyter lab --no-browser --port 8888
EOF
